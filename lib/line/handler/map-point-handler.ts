  ...
  endMatches.forEach(async (match) => {
    if (match.team_home_score !== null && match.team_visitor_score !== null) {
      if (match.team_home_score > match.team_visitor_score) {
        const mapWinnerHome = await prisma.joiner
          .updateMany({
            where: {
        ...
        const mapWinnerVisitor = await prisma.joiner
          .updateMany({
            where: {
        ...
        const mapWinnerDraw = await prisma.joiner
          .updateMany({
            where: {
              team_winner_id: null,
              match_id: match.id
      ...
      const updateMapped = await prisma.match
        .update({
          where: {
            id: match.id
          },
