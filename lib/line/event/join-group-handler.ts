...
export default async function joinGroupHandler(
  event: WebhookEvent
  ...
  const groupProfile: GroupSummaryResponse = await client.getGroupSummary(
    groupId
  )
  ...
  const group = await createGroup(
    ...
    groupProfile.pictureUrl
  )

  const txtMessage: TextMessage = {
    type: 'text',
    text:
      'ขอขอบคุณที่เชิญเราเข้ากลุ่ม: ' +
      groupProfile.groupName +
      ' มาร่วมสนุกทายผลฟุตบอลกับเรากันครับ'
  }
  await client
    .pushMessage(groupId, txtMessage)
    .then(() => {
  ...
  await client
    .pushMessage(groupId, rulMessage)
    .then(() => {
