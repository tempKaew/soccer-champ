...
const replyMatchWhoIsEvent = async (event: WebhookEvent): Promise<MessageAPIResponseBase | undefined> => {
  ...
  if (
    match
    && match.joiner.length!==0
  ) {
    const joinerTeamHome = match.joiner.filter(j => j.team_winner_id === match.team_home_id)
    ...
    const joinerTeamVisitor = match.joiner.filter(j => j.team_winner_id === match.team_visitor_id)
    const mapJoinerTeamVisitor:(typeUserProfile)[] = joinerTeamVisitor.map(j => {
      return {
        id: j.line_users?.id,
        name: j.line_users?.name,
        image: j.line_users?.image
